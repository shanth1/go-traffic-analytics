    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [tree, geo, sum] = await Promise.all([
          analyticsApi.getHierarchy(), // Hierarchy usually static or snapshot, keeping as is
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold ${isLoading ? 'opacity-50' : ''}`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="min-h-[400px]">
          ...
          <CardContent className="h-[350px]">
            {browserStats && !isLoading ? (
              <DonutChart data={browserStats} />
            ) : (
              <div className="h-full flex items-center justify-center text-slate-400">
                {isLoading ? 'Updating...' : 'Loading browser stats...'}
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <CardContent className="h-[400px] w-full overflow-hidden">
          {geoData && !isLoading ? (
            ...
            <div className="p-4 h-full flex items-center justify-center text-slate-400">
              {isLoading ? 'Updating...' : 'Loading map...'}
            </div>
          )}
        </CardContent>
      </Card>
